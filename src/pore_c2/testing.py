...
def simulate_walk(
    ...
    walk_fragments = simulate_walk_path(
        num_monomers, contact_probs, random_state, max_attempt_factor=max_attempt_factor
    )
...
def simulate_sequence_with_cut_sites(
    ...
    length = len(enz.site)
    logger.debug(
        f"Simulating sequence with {len(cut_sites)} cut sites in {seq_length} bases"
    )
...
class SNPHaplotypes:
    ...
    def as_dict(self) -> Dict[str, Any]:
        d = {"chrom": self.chrom, "pos": self.pos, "ref": self.ref, "alt": self.alt}
...
def simulate_sequence_haplotypes(
    ...
    if fragment_df:
        snp_positions, fragment_offsets, fragment_ids = assign_snps_to_fragments(
...
def simulate_concatemers(
    ...
    for size in sizes:
        ...
        while (len(path) < size) and n_iter < max_iter:
            # choose next fragment in proportion to the column for that fragment
            next_frag = random_state.choice(n_frags, p=prob_matrix[:, start_idx])
            if next_frag not in path:
                path.append(next_frag)
            start_idx = next_frag
            n_iter += 1
...
class Scenario:
    ...
    def contact_prob_matrix(self) -> npt.NDArray[np.float_]:
        return simulate_contact_prob_matrix(self.fragments_df, p_cis=self.p_cis)
